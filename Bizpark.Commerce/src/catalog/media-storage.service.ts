...
/** Per-tenant path: `{tenantId}/products/{filename}` */
function objectKey(tenantId: string, filename: string): string {
  return `${tenantId}/products/${filename}`;
}

/**
 * Uploads product images to Supabase Storage (S3-compatible) when configured,
 * falling back to local disk in dev. Mirrors the website-media uploader in
 * Bizpark.API so store-admin product images land in the same bucket.
 */
@Injectable()
export class MediaStorageService {
  ...
  async uploadProductImage(tenantId: string, file: Express.Multer.File): Promise<string> {
    ...
    const supabaseUrl = await this.trySupabaseUpload(tenantId, file);
    if (supabaseUrl) return supabaseUrl;

    return this.saveLocal(tenantId, file);
  }
  ...
  private async trySupabaseUpload(tenantId: string, file: Express.Multer.File): Promise<string | null> {
  ...
  private async saveLocal(tenantId: string, file: Express.Multer.File): Promise<string> {
    const dir = join(this.uploadRoot, tenantId, 'products');
    ...
    const baseUrl = (process.env.COMMERCE_PUBLIC_URL || process.env.PUBLIC_API_URL || `http://localhost:${process.env.PORT ?? 3003}`).replace(/\/$/, '');
    return `${baseUrl}/uploads/${tenantId}/products/${filename}`;
  }
