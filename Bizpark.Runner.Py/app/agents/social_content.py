...
_minimax_image_client: Optional[AsyncOpenAI] = None
...
def _get_minimax_image_client() -> Optional[AsyncOpenAI]:
    global _minimax_image_client
    if _minimax_image_client is None and settings.minimax_api_key:
        _minimax_image_client = AsyncOpenAI(
            api_key=settings.minimax_api_key,
            base_url="https://api.minimaxi.chat/v1",
        )
    return _minimax_image_client


async def _try_generate(client: AsyncOpenAI, model: str, prompt: str) -> str:
    """One image generation attempt — raises on failure."""
    ...
    if not item:
        raise RuntimeError(f"{model} image generation returned no data")
    if item.url:
        return item.url
    if item.b64_json:
        return f"data:image/png;base64,{item.b64_json}"
    raise RuntimeError(f"{model} image response missing url and b64_json")


async def _generate_image_url(prompt: str) -> str:
    """Generate image — OpenAI first, MiniMax fallback, each with one retry."""
    providers = []
    if _get_openai_image_client():
        providers.append(("OpenAI", _get_openai_image_client(), "gpt-image-1"))
    if _get_minimax_image_client():
        providers.append(("MiniMax", _get_minimax_image_client(), "image-01"))

    if not providers:
        raise RuntimeError("No image generation provider available — set OPENAI_API_KEY or MINIMAX_API_KEY")

    last_exc: Exception = RuntimeError("unknown")
    for provider_name, client, model in providers:
        for attempt in range(2):
            try:
                url = await _try_generate(client, model, prompt)
                logger.info(f"Image generated by {provider_name} ({model})")
                return url
